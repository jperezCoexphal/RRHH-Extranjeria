...
new
#[Layout('layouts.app')]
#[Title('Editar Empleador - RRHH Extranjeria')]
class extends Component {
    ...
    {
        $this->employer = $employer->load(['company', 'freelancer', 'address']);

        // Cargar datos empleador
        $this->legal_form = $this->employer->legal_form->value;
        $this->comercial_name = $this->employer->comercial_name;
        ...
        // Cargar datos freelancer
        if ($this->employer->freelancer) {
            $this->first_name = $this->employer->freelancer->first_name ?? '';
            $this->last_name = $this->employer->freelancer->last_name ?? '';
            $this->niss = $this->employer->freelancer->niss ?? '';
            $this->birthdate = $this->employer->freelancer->birthdate?->format('Y-m-d') ?? '';
        }

        // Cargar datos company
        if ($this->employer->company) {
            $this->representative_name = $this->employer->company->representative_name ?? '';
            $this->representative_title = $this->employer->company->representative_title ?? '';
            $this->representative_identity_number = $this->employer->company->representantive_identity_number ?? '';
        }

        // Cargar direccion
        if ($this->employer->address) {
            $this->street_name = $this->employer->address->street_name ?? '';
            $this->number = $this->employer->address->number ?? '';
            $this->floor_door = $this->employer->address->floor_door ?? '';
            $this->postal_code = $this->employer->address->postal_code ?? '';
            $this->country_id = (string) ($this->employer->address->country_id ?? '');
            $this->province_id = (string) ($this->employer->address->province_id ?? '');
            $this->municipality_id = (string) ($this->employer->address->municipality_id ?? '');
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'nif' => 'required|string|max:20|unique:employers,nif,' . $this->employer->id,
    ...
    public function save(): void
    {
        ...
        try {
            $service->update($this->employer->id, $validated);
            session()->flash('success', 'Empleador actualizado correctamente.');
            $this->redirect(route('employers.show', $this->employer->id), navigate: true);
        } catch (\Exception $e) {
            session()->flash('error', 'Error al actualizar el empleador: ' . $e->getMessage());
        }
    }
...
<div>
    @section('page-title', 'Editar Empleador')
        ...
        <div>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-1">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('employers.show', $employer->id) }}">{{ $employer->comercial_name }}</a></li>
                    <li class="breadcrumb-item active">Editar</li>
                </ol>
            </nav>
            <h4 class="text-gray-800 mb-0">Editar Empleador</h4>
        </div>
    </div>
                    ...
                    <div class="card-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" wire:model="ccc" class="form-control @error('ccc') is-invalid @enderror">
                        ...
                        <div class="card-body">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" wire:model="representative_title" class="form-control @error('representative_title') is-invalid @enderror">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="d-grid gap-2">
                            ...
                            <a href="{{ route('employers.show', $employer->id) }}" class="btn btn-outline-secondary">
